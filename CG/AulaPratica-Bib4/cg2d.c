...
so_win InitGraf(bufferdevice * dev) {
  so_win p;
  p.width = dev->MaxX;
  p.height = dev->MaxY;
  ...
  if ((p.display = XOpenDisplay(NULL)) == NULL) {
    ...
    p.dplanes = DisplayPlanes(p.display,p.screen);   
    p.visual = XDefaultVisual(p.display,p.screen);
  
    if (!(p.so_window=XCreateSimpleWindow(p.display,RootWindow(p.display,p.screen),0,0,p.width,p.height,1,BlackPixel(p.display,p.screen),WhitePixel(p.display,p.screen)))) {
      printf("janela do SO: problemas com o ambiente de janelas do X-Window.\n");
      exit(1);
      }
    else {
      XSelectInput(p.display, p.so_window, EventMask);
      XStoreName(p.display, p.so_window, "Monitor Virtual");
      p.gc = XCreateGC(p.display, p.so_window, 0, &(p.values));
      XMapWindow(p.display,p.so_window);
      XSync(p.display,False); 
      p.ximage = XCreateImage(p.display,p.visual,p.dplanes,ZPixmap,0,malloc(p.width*p.height*sizeof(int)),p.width,p.height,8,0);
      }
    }
...
int RefreshX(bufferdevice * dev, palette * pal, so_win p) {
  ColorValues  * cor;
  float r, g, b;

  for(int m=0;m<p.height;m++) {
    for(int n=0;n<p.width;n++) {
       if (dev->buffer[m*p.width+n] < 0) {
          b = 0.0;
          g = 0.0;
          r = 0.0;
          }
       else {        	
          cor = GetColor(dev->buffer[m*p.width+n],pal);
          b = cor->blue;
          g = cor->green;
          r = cor->red;
          }     
       p.ximage -> data[(m*4)*p.width+n*4+0] = (char) round(b*255);
       p.ximage -> data[(m*4)*p.width+n*4+1] = (char) round(g*255);
       p.ximage -> data[(m*4)*p.width+n*4+2] = (char) round(r*255);
       p.ximage -> data[(m*4)*p.width+n*4+3] = (char) 0;
       }
    }
    
  XPutImage(p.display,p.so_window,p.gc,p.ximage,0,0,0,0,p.width,p.height);
